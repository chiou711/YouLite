...
public class ExportSel_toGDriveJsonFragment extends Fragment {
	Context mContext;
	TextView title;
	...
	public ExportSel_toGDriveJsonFragment(){}
	...
	public class ShowProgressBarAsyncTask extends AsyncTask<Void, Integer, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			//get data from DB
			try {
				jsonStr = util.queryJsonDB(mStrSDCardFileName.replace(".json",""),mList_selPage.mCheckedTabs);
...
//			getActivity().getSupportFragmentManager().popBackStack();
			Intent intSel = new Intent(getActivity(), ExportSelGDriveAct.class);
			intSel.putExtra("FILE_NAME",mStrSDCardFileName);//todo Add file name
			intSel.putExtra("SEL_JSON",jsonStr);
			startActivity(intSel);
		}
	}
}