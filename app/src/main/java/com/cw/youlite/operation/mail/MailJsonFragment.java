...
public class MailJsonFragment extends Fragment{
	Context mContext;
	TextView title;
	Intent mEMailIntent;
	        ...
	        if(strEMailAddr.length() > 0)
	        {
				...
				if((list_selPage.isCheckAll) ||
				   (list_selPage.mListStrArr.size() == countChecked) )
					defaultFileName = list_selPage.mFolderTitle;// + ".json";
				else
					defaultFileName = pagesName;// + ".json";
		        ...
		        if(extras == null)
		        {
		            // JSON file
			        try {
				        mEMailBodyString = util.exportToSdCardJson(attachmentFileName[0], // attachment name
